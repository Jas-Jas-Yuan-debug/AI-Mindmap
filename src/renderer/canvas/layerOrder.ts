...
// unit-testable. The nodes store's `reorderLayer(action, ids)` action calls
// `reorderLayer(...)` here and swaps in the result; the properties panel wraps
// that store action in one `useHistory.transact(...)` step for undo grouping.
//
// Design notes:
