...
//
// Contract (sibling A — style schema + resolver): NodeBase gains the optional
// fields above. We import PRESET_COLOR_MAP for the preset hex swatches. After
// A lands, the canonical home for the style fields/preset map may be
// `../canvas/nodes/nodeStyle.ts` — the import is verified on rebase. Until
// then PRESET_COLOR_MAP lives in TextNode.tsx (re-exported), which is where A
// is moving it from, so this import keeps working either way.
//
...
import type { AimapNode, Color, PresetColor } from "../store/nodes.js";
...
// ---------------------------------------------------------------------------
// Style field types (contract with sibling A). Declared locally so this file
// type-checks before A's schema lands; A's `NodeBase` widens with the same
// field names + value domains. On rebase these stay structurally compatible.
// ---------------------------------------------------------------------------

type StrokeWidth = 1 | 2 | 4;
type StrokeStyle = "solid" | "dashed" | "dotted";
type Roundness = "sharp" | "round";

/** The styleable fields the panel writes. Mirror of A's NodeBase additions. */
...
type StyledNode = AimapNode & StyleFields;
...
export function PropertiesPanel() {
  ...
  /** Reorder the selected node(s) within the nodes array (z-order). */
  const applyLayer = (action: LayerAction) => {
    useHistory.getState().transact(() => {
      useNodes.setState((s) => ({
        nodes: reorderLayer(action, s.nodes, selectionIds),
      }));
    });
  };
  ...
  const widthActive: StrokeWidth =
    (primary.strokeWidth as StrokeWidth | undefined) ?? 1;
