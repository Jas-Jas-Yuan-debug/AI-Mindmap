...
**Phase 5 status: 🟢 DONE (2026-05-24) — 4 / 4 criteria met.** Closed by PR #32 (persistence engine), PR #33 (file menu UX), and PR #34 (autosave + dirty indicator + unsaved-changes prompt + friendly corrupt-file error dialog).
...
**Phase 5 PR 3/3 (PR #34) — autosave + dirty indicator + unsaved-changes prompt + corrupt-file errors (sibling subagent C).** Adds the document-lifecycle UX on top of the engine + file menu: (1) `src/renderer/store/docStatus.ts` — a tiny Zustand slice (`dirty`, `lastSavedAt`, `markDirty()`, `markSaved()`) whose `installDocStatusSubscriptions()` (called once from `main.tsx` after first render) flips `dirty` on any nodes/edges/viewport change; `fileActions` calls `markSaved()` on New/Open/Save/Save As. (2) `src/renderer/persistence/useAutosave.ts` — mounted in `App`, debounces 1000ms and writes back through the existing `saveDocument()` seam, only when a backing file handle exists (Untitled docs are never auto-written). (3) `src/renderer/persistence/useDocumentTitle.ts` — reflects filename + a `•` dirty bullet into `document.title` (`AI-Mindmap — <name> •`). (4) `src/renderer/ui/UnsavedChangesDialog.tsx` implements the `window.__aimConfirmDiscard` seam (Save / Don't Save / Cancel themed modal) that `fileActions` now `await`s before New/Open when dirty; window close is also guarded (web: `beforeunload`; Electron: renderer reports dirty over a new `window:dirtyChanged` channel and the main process defers close to the same in-app prompt via `window:confirmClose`). (5) `src/renderer/ui/ErrorDialog.tsx` implements the `window.__aimReportFileError` seam with friendly messages (invalid JSON vs. specific Zod field path, via the pure `persistence/fileErrorMessage.ts`). 27 new unit tests (docStatus transitions + subscriptions, autosave decision + debounce with fake timers, title format, error-message mapping). The `__aimConfirmDiscard` seam contract was widened from `() => boolean` to `() => boolean | Promise<boolean>` and `fileActions.newDocument`/the guard made async so the modal can resolve before discard.
...
- [x] Autosave debounce works (rapid edits don't hammer disk) — *closed by PR #34. `store/docStatus.ts` flips a `dirty` flag on any nodes/edges/viewport edit; `persistence/useAutosave.ts` debounces 1000ms (`AUTOSAVE_DEBOUNCE_MS`), re-arming on every edit so rapid edits collapse to a single `saveCanvas` write, and only autosaves when a backing file handle exists (an Untitled doc is never auto-written). Pure `shouldAutosave()` decision + the debounce loop are unit-tested with fake timers.*
- [x] Corrupt JSON shows error, doesn't crash; partial-corrupt (valid JSON, fails Zod) shows specific field error — *closed by PR #34 (ErrorDialog + Zod issue surfacing). `ui/ErrorDialog.tsx` implements the `window.__aimReportFileError` seam fileActions calls; `persistence/fileErrorMessage.ts` maps the raw error to friendly copy: invalid JSON → "This file isn't a valid .aimap file."; valid-JSON-but-Zod-invalid → the specific field path + message (e.g. `nodes.0.width: Expected number, received string`) extracted from the `MigrationError`/`parseAimapFile` envelope. The app never crashes — the modal replaces the console+alert fallback. Engine pieces from PR #32 (`migrate` throws on corrupt JSON, `parseAimapFile` returns structured `issues[]`).*
...
**Estimated PRs:** 3 (PR #32 = engine PR 1/3; PR #33 = file menu PR 2/3; PR #34 = autosave/dirty/prompt/errors PR 3/3 — all landed)
...
**Exit criteria**
- [x] Nested groups work (group inside a group) (closed by PR #36 — a group can be dropped into another group; `descendantsOf`-driven group-move carries nested subtrees; `groupDropTarget` picks the innermost group on a drop; Canvas z-order renders groups in ascending nesting depth so a child group paints on top of its parent; 22 new unit tests across `groupHitTest.test.ts` + `reparent.test.ts`)
- [x] Cycle prevention: can't make A child of B if B is descendant of A (closed by PR #35 — `wouldCreateCycle` guards `setParent` in `src/renderer/store/reparent.ts`; refuses self-parent, direct-descendant, and transitive-descendant reparents and leaves the store untouched; 28 unit tests in `reparent.test.ts`)
- [x] Save/load preserves group hierarchy (closed by PR #37 — `serialize.ts` passes the node array through whole, so `parentId` / GroupNode `label` / GroupNode `collapsed` already round-trip; verified by a new `src/shared/serialize.test.ts` that round-trips a nested-group hierarchy through `toAimapFile` → `fromAimapFile` AND through `JSON.stringify` + `parseAimapFile` (Zod, the disk path), asserting parentId chain + label + collapsed + z-order all survive)
- [x] Undo/redo works for re-parenting (closed by PR #37 — `transact(() => setParent(child, group))` is one undoable + redoable step; a reparent that rides with a move collapses into the SAME step; label / color / collapse each capture once = one undo step; new `undo / redo of re-parenting` block in `reparent.test.ts`)

**Phase 6 status: 4/4 criteria met — 🟢 done (2026-05-24).** Closed by PR #35 (GroupNode foundation + cycle guard), PR #36 (nested groups + drag/reparent + group-move), and PR #37 (label/color/collapse + save-load hierarchy + undo reparent).

**Phase 6 PR 3/3 (PR #37) — group label/color/collapse + save-load hierarchy + undo reparent (sibling subagent C).** **Label editing**: double-click a group's HEADER strip → an HTML `<input>` overlay positioned over the header (new `src/renderer/ui/GroupOverlayLayer.tsx`, mirroring NodeOverlayLayer's screen-tracking pattern); commit on Enter/blur → `updateNode(groupId,{label})` wrapped in one `capture()` (empty label clears the field; Konva header falls back to "Group"). The Konva header `Text` left by B is now the read-mode display. **Color**: right-click anywhere on a group → the shared `ColorPicker` (`targetKind:"node"` → `updateNode(id,{color})`, one capture) — NodeOverlayLayer's contextmenu only matches TEXT nodes, so groups needed their own entry point in GroupOverlayLayer. **Collapse**: a disclosure chevron in the header (Konva `Line` + transparent hit Rect) toggles `collapsed` (one capture); when collapsed the header shows the hidden-descendant count ("Group (5)") and `Canvas.tsx` filters the subtree out of BOTH render passes via the new pure helper `isHiddenByCollapsedAncestor(nodes, nodeId)` in `reparent.ts` (walks the parentId chain for a collapsed ancestor; the group itself stays visible; loop-guarded). Hidden nodes are absent from the Konva tree so they neither paint nor hit-test. **Save/load + undo** verified by tests (see exit criteria). Overlay scope is cleanly split: label-edit is GROUPS-ONLY (GroupOverlayLayer), markdown-edit is TEXT-ONLY (NodeOverlayLayer). New files: `ui/GroupOverlayLayer.tsx`, `shared/serialize.test.ts`. Edited: `canvas/nodes/GroupNode.tsx` (chevron + count + read-label; B's drag/resize handlers untouched), `canvas/Canvas.tsx` (collapsed-hide filter), `store/reparent.ts` (additive `isHiddenByCollapsedAncestor`), `store/reparent.test.ts` (+16 tests), `App.tsx` (mount GroupOverlayLayer). 20 new unit tests (16 in `reparent.test.ts`, 4 in `serialize.test.ts`).

**Phase 6 PR 2/3 (PR #36) — drag in/out of group + group-move + nested groups (sibling subagent B).** New pure geometry `src/renderer/canvas/interactions/groupHitTest.ts` (`pointInBox` / `centerOf` / `intersectionArea` / `isMostlyInside` / `groupDropTarget`, no Konva/DOM) + store-aware glue `dropReparent.ts` (`reparentOnDrop` → routes a drop's center hit-test through A's cycle-safe `setParent`). **Drag a node onto a group** sets its `parentId` (TextNode `onDragEnd` calls `reparentOnDrop`); **drag it out over empty canvas** clears it. **Drag a group** moves its whole subtree: extended GroupNode `onDragStart`/`onDragMove` at A's `NOTE(claude/phase-6-A)` marker to snapshot `descendantsOf(group)` and apply the group's delta to every descendant (nested children included) — one undo step (capture on drag start). **Resize a group** added the 8-handle affordance to GroupNode (clamped to `GROUP_MIN_*`); children are NOT resized, and on resize-end any DIRECT child no longer `isMostlyInside` (≥50% area) the new bounds is detached via `setParent(child, null)`. **Nested groups**: a group can be dropped into another (cycle guard + own-subtree exclusion in `reparentOnDrop` prevent loops); Canvas z-order pass now sorts groups by nesting depth (new pure `depthOf` in `reparent.ts`) so a child group paints on top of its parent while text still paints above all groups. 22 new unit tests (`groupHitTest.test.ts` 18, `reparent.test.ts` +4 for `depthOf`). Files touched (sibling C awareness): `canvas/nodes/GroupNode.tsx` (drag/resize handlers + handles; header `Text` untouched for C's label editing), `canvas/nodes/TextNode.tsx` (`onDragEnd` reparent), `canvas/Canvas.tsx` (group z-order sort), `store/reparent.ts` (additive `depthOf` export). Did NOT tick save/load-hierarchy or undo-reparent — those are sibling C.

**Phase 6 PR 1/3 (PR #35) — GroupNode foundation (sibling subagent A).** Widened the runtime node union `AimapNode` from `TextNode` to `TextNode | GroupNode` in `src/renderer/store/nodes.ts` (file/link/image still join in Phase 7) + added `makeGroupId()`; guarded the markdown overlay (`ui/NodeOverlayLayer.tsx`) to operate only on text nodes so groups get no DOM overlay. New `src/renderer/canvas/nodes/GroupNode.tsx` (`GroupNodeBox`): a dashed, faintly-tinted, titled rounded-rect container with a static header label (sibling C makes it editable) and a minimal self-drag (mirrors x/y to `moveNode`, one undo step; sibling B extends it to move children). Z-order in `Canvas.tsx`: a two-pass render in the Nodes Layer draws group containers FIRST (behind), then text cards (on top) — chosen over re-sorting so the store array order (= document z-order, plan §5) stays untouched. New `src/renderer/store/reparent.ts` with the cycle-safe reparent primitives siblings B/C build on: `isDescendant` / `wouldCreateCycle` / `childrenOf` / `descendantsOf` (pure over a node array, loop-guarded) + `setParent(childId, parentId|null)` store action (the ONLY sanctioned `parentId` mutation; refuses cycles, returns boolean). Dev helper `window.__aimAddGroup(x,y,w,h,label?)` (`dev/aimAddGroup.ts`, DEV-only). 28 tests in `reparent.test.ts`. Did NOT tick nested-groups / save-load / undo — those are siblings B and C.
...
- 2026-05-24: PR #34 — Phase 5 (PR 3/3): autosave + dirty indicator + unsaved-changes prompt + friendly corrupt-file error dialog. `store/docStatus.ts` (dirty flag + `installDocStatusSubscriptions`), `persistence/useAutosave.ts` (1000ms debounce, file-handle gated), `persistence/useDocumentTitle.ts` (`AI-Mindmap — <name> •`), `ui/UnsavedChangesDialog.tsx` (the `__aimConfirmDiscard` seam + web `beforeunload` + Electron `window:dirtyChanged`/`window:confirmClose` close guard in `main.ts`/`preload.ts`), `ui/ErrorDialog.tsx` + `persistence/fileErrorMessage.ts` (the `__aimReportFileError` seam: invalid-JSON vs. specific Zod field path). Widened the `__aimConfirmDiscard` contract to allow a Promise and made `newDocument`/the discard guard async. 27 new tests. Ticked §6 Phase 5 "Autosave debounce works" + "Corrupt JSON shows error … specific field error" → **Phase 5 🟢 4/4 done** (closed by PR #32 engine + PR #33 file menu + this PR).
- 2026-05-24: PR #37 — Phase 6 (PR 3/3): group label/color/collapse + save-load hierarchy + undo reparent (sibling C). Label editing via a new `ui/GroupOverlayLayer.tsx` (double-click header → HTML `<input>` over the header strip → `updateNode(id,{label})` on Enter/blur, one capture; empty clears). Right-click a group → shared `ColorPicker` (`targetKind:"node"`, one capture) since NodeOverlayLayer's contextmenu only matches text nodes. Collapse: a disclosure chevron in the Konva header toggles `collapsed` (one capture); collapsed header shows the hidden-descendant count ("Group (5)") and `Canvas.tsx` filters the subtree out of both render passes via new pure `isHiddenByCollapsedAncestor(nodes,nodeId)` in `reparent.ts` (so hidden nodes neither paint nor hit-test; the group itself stays visible; loop-guarded). Save/load hierarchy: `serialize.ts` already round-trips `parentId`/`label`/`collapsed` (passes the node array through whole) — verified by new `shared/serialize.test.ts` (nested-group hierarchy through `toAimapFile`→`fromAimapFile` AND `JSON.stringify`+`parseAimapFile`). Undo reparent: `transact(()=>setParent(...))` is one undoable+redoable step (a reparent riding with a move collapses into the same step) — verified by a new block in `reparent.test.ts`. 20 new unit tests (16 reparent, 4 serialize). Overlay scope split kept clean: label-edit groups-only, markdown text-only. Files: new `ui/GroupOverlayLayer.tsx` + `shared/serialize.test.ts`; edited `canvas/nodes/GroupNode.tsx` (chevron+count+read-label; B's drag/resize untouched), `canvas/Canvas.tsx` (collapse filter), `store/reparent.ts` (additive helper), `App.tsx` (mount). Ticked §6 Phase 6 "Save/load preserves group hierarchy" + "Undo/redo works for re-parenting" → **Phase 6 🟢 4/4 done** (closed by PR #35 + #36 + this PR).
- 2026-05-24: PR #36 — Phase 6 (PR 2/3): group drag/reparent (sibling B). New pure geometry `canvas/interactions/groupHitTest.ts` (`pointInBox`/`centerOf`/`intersectionArea`/`isMostlyInside`/`groupDropTarget`) + store glue `dropReparent.ts` (`reparentOnDrop`). Drag a node onto a group → `setParent` (center hit-test); drag out → detach. Drag a group → its whole `descendantsOf` subtree moves by the same delta (one undo step; extended A's `NOTE(claude/phase-6-A)` marker in `GroupNode.tsx onDragMove`). Added the 8-handle resize affordance to groups (clamped to `GROUP_MIN_*`); children aren't resized, and on resize-end children no longer ≥50%-inside the new bounds are detached. Nested groups: a group can be dropped into another (own-subtree excluded + cycle guard); Canvas z-order now sorts groups by nesting `depthOf` (new pure helper in `reparent.ts`) so a child group paints above its parent. 22 new unit tests. Ticked §6 Phase 6 "Nested groups work" → Phase 6 2/4. Files touched for sibling C awareness: `GroupNode.tsx` (header `Text` left intact for C's label editing), `TextNode.tsx`, `Canvas.tsx`, `store/reparent.ts` (additive `depthOf`). Open for sibling C: save/load hierarchy + undo reparent.
- 2026-05-24: PR #35 — Phase 6 (PR 1/3): GroupNode foundation. Widened runtime `AimapNode` to `TextNode | GroupNode` + `makeGroupId` (`store/nodes.ts`); guarded the markdown overlay to text-only (`ui/NodeOverlayLayer.tsx`). New `canvas/nodes/GroupNode.tsx` (`GroupNodeBox` — dashed titled tinted container, static label, minimal self-drag) wired into `Canvas.tsx` with a two-pass z-order (groups behind, text on top). New `store/reparent.ts` (`isDescendant`/`wouldCreateCycle`/`childrenOf`/`descendantsOf` pure helpers + cycle-refusing `setParent` store action). Dev helper `window.__aimAddGroup`. 28 tests in `reparent.test.ts`. Ticked §6 Phase 6 "Cycle prevention" criterion → Phase 6 1/4. Open for siblings B (nested groups / drag in-out / group-move) + C (label/collapse / save-load hierarchy / undo reparent).
- 2026-05-26: PR #48 — Phase 8: Excalidraw-style properties panel (`ui/PropertiesPanel.tsx` + `.css`), mounted left-edge in `App.tsx` below the main menu (chat sidebar owns the right edge), visible only when `useSelection().ids` is non-empty. Writes the per-node style fields landed by the dark-mode-fix foundation: stroke / background / font color (6 `PRESET_COLOR_MAP` swatches + `<input type=color>` custom; background also has a transparent/none chip that clears `backgroundColor`), stroke width 1/2/4, border style solid/dashed/dotted, corners sharp/round, opacity 0–100. Reflects the first selected node's current values. Layer (z-order) section — bring-to-front/forward/backward/send-to-back — added as a `reorderLayer(action, ids)` action on the nodes store, implemented over pure `canvas/layerOrder.ts` transforms (array order == z-order, plan §5). Every control applies to ALL selected nodes as ONE undo step via `useHistory.transact`. Tests: 17 in `layerOrder.test.ts` (pure transforms) + new `reorderLayer` block in `nodes.test.ts` (store action). No plan-contract changes — uses the existing `NodeBase` style fields and `useNodes`/`useHistory`/`useSelection` APIs.
