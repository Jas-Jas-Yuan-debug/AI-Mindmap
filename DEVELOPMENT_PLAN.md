...
**Exit criteria**
- [x] Nested groups work (group inside a group) (closed by PR #PRNUM — a group can be dropped into another group; `descendantsOf`-driven group-move carries nested subtrees; `groupDropTarget` picks the innermost group on a drop; Canvas z-order renders groups in ascending nesting depth so a child group paints on top of its parent; 22 new unit tests across `groupHitTest.test.ts` + `reparent.test.ts`)
...
Last updated: 2026-05-24 (Phase 6 PR 2/3 — group drag/reparent: drag node in/out of group, drag group moves subtree, resize detaches escapees, nested groups; Phase 6 2/4)
